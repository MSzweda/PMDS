...
public class MDeviceScheduleActivity extends ExpandableListActivity 
{
	...
	*/
	...
	private ArrayList<GroupItem> convertGroupCursorToList(Cursor cursor)
	{
		...
		{
			int pom = cursor.getInt(cursor.getColumnIndex(ManualSchedulesDBManager.DEVICE_TYPE));
	...
	private ArrayList<ChildItem> convertChildrenCursorToList(int devType)
	{
		...
		while(!cur.isAfterLast())
		{
			...
			children.add(new ChildItem(
					...
					getDayString(cur.getInt(cur.getColumnIndex(ManualSchedulesDBManager.WEEKDAY))),
					...
					getSetToString(cur.getInt(cur.getColumnIndex(ManualSchedulesDBManager.ACTION_TYPE)),devType),
					devType,
